...
def merge_events(events_df, max_gap=30):
    ...
    for i in range(1, len(events_df)):
        ...
        same_subsystem = (
            current["subsystem"] == next_event["subsystem"]
        )

        # Compare event type
        same_type = (
            str(current["event_type"]) == str(next_event["event_type"])
        )
        ...
        if same_subsystem and gap <= max_gap:
            ...
            current["rule_detected"] = (
                current["rule_detected"]
                or next_event["rule_detected"]
            )

            current["ml_detected"] = (
                current["ml_detected"]
                or next_event["ml_detected"]
            )
